...
(function() {
    fdTableSort = {
        ...
        initEvt: function(e) {
            if(e != undefined) {  /* OL, update to avoid unexpected re-initialisation */
                fdTableSort.init(false);
            }
			return ;  /* OL, update to avoid unexpected re-initialisation */
        },
                ...
                for(c = 0;c < workArr.length;c++) {
                    for(i = 0;i < workArr[c].length;i++){
                        ...
                        if(workArr[c][i].className.match('sortable')) {
                            ...
                            /*** Olivier Lequeux ack for help : BEGIN
                                /** get td.rel content as <a> tooltip */
                                help = workArr[c][i].getAttribute('rel');
                                if(help!=null) {
                                    aclone.title = help;
                                }
                                // aclone.title = "Sort on \u201c" + thtext.replace('<br />', '') + "\u201d";
                            // *** Olivier Lequeux ack for help : END
        ...
        addClass: function(e,c) {
			...
			else return;  /* OL, update to avoid unexpected re-initialisation */
        },
        /*@cc_on
        /*@if (@_win32)
